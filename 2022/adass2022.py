#! /usr/bin/env python
#
#   ADASS 2020 sample processing of the 3 XLS spreadsheets
...
class adass(object):
    def __init__(self, dirname='.', debug=True):
    ...
    def getcsv(self):
        ...
        for row in csvreader: 
            ...
            print(row[0],row[1])
        print("Found %s records in %s" % (len(self.rows),self.p1))
    ...
    def split_name(self,fullname):
        """ For a fullname like "First Last" return (first,last)
    ...
    def tab2list(self, filename, use_code=False):
        ...
        nz = 0   # count contributions with no catagory
        nl = 0
        for i in range(len(o1)):
            ...
            nl = nl + 1
            if code in self.names.keys():
                print(code,self.names[code])
            else:
                print(code," Warning: not in CSV but in %s" % filename )
    ...
    def report_3b(self,o1,o2,o3, count=False, dirname='.'):
        ...
        if True:
            print("report_3b skipping for 2020")
            return
