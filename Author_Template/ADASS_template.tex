% This is the ADASS_template.tex LaTeX file, 24 Oct 2025.
...
% pdflatex ADASS_template
% pdflatex ADASS_template
% This will create a file called ADASS_template.pdf

\documentclass[11pt,twoside]{article}

% Do NOT use ANY packages other than asp2021. 
\usepackage{asp2021}
...
\title{ADASS Author Template with Long Title}
...
\begin{abstract}
This is the ADASS 2025 author template file.  This is based on the
Astronomical Society of the Pacific (ASP) 2021 template, but has been
modified to reflect the specific ADASS requirements.
\end{abstract}
...
\item No use of \verb"\usepackage" except for \verb"\usepackage{asp2021}". (See LaTeX packages and commands.)
...
\item Graphics files have to be .jpg or .png (this is new since 2025, they used to be .eps)
\item Name all the files properly:- figures are <paper>\_f<n>.jpg, eg O1-3\_f1.png. Paper names use dashes not periods, O1-3.tex not O1.3.tex. Posters now use the same naming convention as oral papers, e.g.\ X3-21. (See File names and Paper IDs.)
...
These figures have been converted to the `png' image format.  Notice
that the references to the figures do not need to include the file
extension and that the pdfLaTeX compiler recognizes the non-`eps'
image files.

 
\articlefigure[width=0.65\textwidth]{example.jpg}{ex_fig1}{Welcome to 1365.  Size adjusted to be a little smaller than full.}
...
\articlefiguretwo{example}{example}{ex_fig2}{Now there are two of them.  \emph{Left:} An image from long ago.  \emph{Right:} The same exact thing.}
...
\articlefigurefour{example}{example}{example}{example}{ex_fig3}{Now four of them?}
...
% \bookpartphoto[width=1.0\textwidth]{foobar.jpg}{FooBar Photo (Photo: Any Photographer)}
